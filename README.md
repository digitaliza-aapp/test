...
Script (```install_frame_webkit_kiosk.sh```) para desplegar la interfaz gráfica de un quiosco interactivo  utilizando el paquete snap ```wpe-webkit-mir-kiosk``` mediante ```ubuntu-frame``` en un servidor ```Ubuntu Server LTS```.
...
[**wpe-webkit-mir-kiosk**](https://gitlab.com/glancr/wpe-webkit-snap) Fork de [**WPE WebKit**](https://wpewebkit.org/) empaquetado en snap para usar en quioscos a pantalla completa
